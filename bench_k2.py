...
async def run_benchmark():
    print("📊 K2-THINK V2 PERFORMANCE BENCHMARK")
    print("====================================")
    
    # K2 only has one model for now, but we structure it as a list for consistency
    models = [
    ...
    # Initialize System with K2 Provider
    print("→ Initializing ARVIS Core (K2 Provider)...")
    ...
    for model in models:
        ...
        try:
            ...
            # Use UnifiedLLM (which routes to K2 based on env var)
            # OpsCopilot initializes LLM once, but since we set env var before init, it should be K2.
            # We can verify:
            print(f"   (Using Provider: {copilot.llm_agent.llm._REASONING_AGENT.provider})")
